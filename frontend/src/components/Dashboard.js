...
export default function Dashboard({ games, user }) {
  ...
  const fetchAllUsers = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/all-users/${currentUser.email}`);
  ...
  const refreshUser = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/games/${currentUser.email}`);
  ...
  const updateGame = async (slipIndex, gameIndex, field, value) => {
    ...
    if (currentUser.role === "admin") {
      try {
        await axios.post("http://localhost:5000/update-game", {
  ...
  const approveUser = async email => {
    await axios.post("http://localhost:5000/approve-user", { adminEmail: currentUser.email, userEmail: email });
    fetchAllUsers();
  };

  const activateUser = async (email, plan) => {
    try {
      await axios.post("http://localhost:5000/activate", { adminEmail: currentUser.email, userEmail: email, plan });
  ...
  const updateSlipType = async (slipIndex, type) => {
    ...
    try {
      await axios.post("http://localhost:5000/update-slip-type", { adminEmail: currentUser.email, slipIndex, type });
  ...
  const updateNewGame = (i, field, value) => { const games = [...newSlip.games]; games[i][field] = value; setNewSlip({ ...newSlip, games }); };
  ...
  const saveSlip = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/add-slip", { adminEmail: currentUser.email, slip: slipToSave });
                  ...
                  <td>
                    {!u.premium ? (
                      <select onChange={e => activateUser(u.email, e.target.value)} defaultValue="">
                        ...
                        {Object.keys(subscriptions).map(p => (
